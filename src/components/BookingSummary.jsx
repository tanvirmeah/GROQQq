...
const BookingSummary = () => {
  ...
  let minibarTotal = 0;
  if (booking.booking_details && booking.booking_details.minibarItems) {
    for (const itemId in booking.booking_details.minibarItems) {
      const item = minibarItems.find(i => i.id === itemId);
      if (item) {
        minibarTotal += item.price * booking.booking_details.minibarItems[itemId];
      }
    }
  }
  ...
  return (
    <div className="booking-summary">
      ...
      {minibarItems.length > 0 && booking.booking_details && booking.booking_details.minibarItems && Object.keys(booking.booking_details.minibarItems).length > 0 && (
        <section className="summary-section">
          <h3>Minibar Items</h3>
          <table className="summary-table">
            <thead>
            ...
            <tbody>
              {Object.entries(booking.booking_details.minibarItems).map(([itemId, quantity]) => {
                const item = minibarItems.find(i => i.id === itemId);
                if (!item) return null;
